...
class ProfileEditView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
     ...
     class ServicePriceEditView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
          model = ServicePrice
          fields = ['price']  # Update with the desired fields for editing service price
          template_name = 'service_price_edit.html'

          def get_success_url(self):
               service_price_pk = self.object.pk  # Retrieve the service price's primary key
               return reverse('caretaker:service-price-detail', kwargs={'pk': service_price_pk})
          
          def test_func(self):
               service_price = self.get_object()
               return self.request.user == service_price.user