...
def display(request):
    ...
    if selected_name:
        profiles = profiles.filter(name__icontains=selected_name)
    
    profiles = profiles.order_by('-view_counter')  # Sort by view_counter in descending order
    
    cities = UserProfile.objects.values_list('city', flat=True).distinct()
    names = UserProfile.objects.values_list('name', flat=True).distinct()
...
def account_redirect(request):
    return redirect('caretaker:profile', pk=request.user.pk)
...
class ProfileView(View):
    def get(self, request, pk, *args, **kwargs):
          ...
          user = profile.user
          service_descriptions = profile.servicedescription_set.all()
          profile.view_counter += 1
          profile.save()
          context = {
               'user': user,
               'profile': profile,
...
class ProfileEditView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
     ...
     def get_success_url(self):
          ...
          return reverse('caretaker:profile', kwargs={'pk': profile_pk})
...
from .models import UserProfile, ServiceDescription
from .forms import ServicePriceForm

class ServicePriceEditView(View):
     def get(self, request, pk):
          service_description = get_object_or_404(ServiceDescription, pk=pk)
          form = ServicePriceForm(instance=service_description)
          return render(request, 'service_edit.html', {'form': form})
     ...
     def post(self, request, pk):
          ...
          if form.is_valid():
               form.save()
               return redirect('caretaker:profile', pk=service_description.user_profile.pk)
          return render(request, 'service_edit.html', {'form': form})
